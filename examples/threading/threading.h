...
struct thread_data{
     /**
      * Time to wait until obtaining the mutex in us
      */
    int wait_to_obtain_us;

    /**
     * Time to wait until releasing the mutex in us
     */
    int wait_to_release_us;

    /**
     * Mutex to use in the thread
     */
...
typedef struct thread_data thread_data_s;
